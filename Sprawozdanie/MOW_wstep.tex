...
Po zbudowaniu modelu na podstawie zbioru trenującego przeprowadzone zostaną następujące eksperymenty:
...
Celem analizy wyników jest stwierdzenie, która z metod jest najbardziej odpowiednia dla badanych zbiorów danych. 
...
% Opis oceny anomalii dałabym w tym podpunkcie

W celu oceny algorytmu detekcji anomalii konieczne jest użycie niestandardowych wskaźników, które nie tylko swierdzą obecność anomalii, ale również określą jej wlaściwości. Pierwszym z proponowanych wskaźników jest CBLOF liczony jako iloczyn odległości badanej probki od najbliższej dużej grupy a liczbą elementów w grupie, do której obiekt został zaklasyfikowany. Koncepcja małych i dużych grup nie jest precyzyjnie określona - możliwy jest wybór algorytmu podziału.
...
Lepszym rozwiązaniem będzie zatem być może nieważony wskaźnik CBLOF oparty jedynie na odległości od grup, z pominięciem ich liczności. W projekcie zostaną zastosowane obie wersje ocen anaomalii i wykonane zostanie porównanie między nimi.
...
W celu oceny jakości klasyfikacji za pomocą metod drzew decyzyjnych oraz k najbliższych sąsiadów wyznaczone zostaną macierze pomyłek.


\begin{figure}[ht]
%\vspace{-20pt}
\centering
\includegraphics[width=50mm,scale=0.5]{conf_matrix}
\caption{Szablon binarnej macierzy pomyłek.}
\label{conf_matrixp}
\end{figure}
